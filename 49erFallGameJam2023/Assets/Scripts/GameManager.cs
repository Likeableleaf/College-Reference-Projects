...
public class GameManager
{
    ...
    public List<Slowbody> robots = new List<Slowbody>();
    ...
    public void nextScene()
    {
        int i = 0;
        if (robots.Count > 0)
            foreach (Robot robot in robotPrefabs)
            {
                //set robot alive to be the inverse of robot broken. This is how we determine if we are spawning every robot.
                if (robot.alive)
                {
                    robot.alive = !robots[i].broken;
                    i++;
                }
                //was
                /*                if (robot.alive)
